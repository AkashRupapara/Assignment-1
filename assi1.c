/*  
 *  assi1.c - Kernel module for CMPE283 assignment 1
*/
 ...
 * SDM volume 3C, section 24.6.2 ref Table 24-8
 */
 ...
 * SDM volume 3, section 24.6.2 ref Table 24-7
 */
...
detect_vmx_features(void)
{
	...
	report_capability(procbased, 22, pri_proc_lo, pri_proc_hi);
    ...
    if(1 << 31 & pri_proc_hi){
        printk("**** Secondary procbased controls - AVAILABLE!! ****");
                    ...
                    pr_info("Secondary procbased Controls MSR: 0x%llx\n",
                        (uint64_t)(lo | (uint64_t)hi << 32));
                    report_capability(secondary_procbased, 22, lo, hi);
